...
var verificationCmds = map[finding.CheckID]string{
	...
	finding.CheckGHActionWorkflowRunUnsafe:     "grep -r 'workflow_run' .github/workflows/",
	finding.CheckGHActionGitHubEnvInjection:    `grep -r 'GITHUB_ENV\|GITHUB_PATH' .github/workflows/`,
	finding.CheckGHActionSecretsInherit:        "grep -r 'secrets: inherit' .github/workflows/",
	finding.CheckGHActionInsecureCommands:      "grep -r 'ACTIONS_ALLOW_UNSECURE_COMMANDS' .github/workflows/",
	finding.CheckGHActionBotConditionSpoofable: "grep -r 'github.actor ==' .github/workflows/",
	finding.CheckGHActionArtiPacked:            `grep -r 'actions/checkout\|upload-artifact' .github/workflows/`,
	finding.CheckGHActionCachePoisoning:        `grep -r 'actions/cache\|release:' .github/workflows/`,
	finding.CheckGHActionAWSLongLivedKey:       `grep -r 'AWS_ACCESS_KEY_ID\|AWS_SECRET_ACCESS_KEY' .github/workflows/`,
	finding.CheckGHActionGCPServiceAccountKey:  `grep -r 'GOOGLE_CREDENTIALS\|GCP_SA_KEY' .github/workflows/`,
	finding.CheckGHActionAzureCredentials:      `grep -r 'AZURE_CREDENTIALS\|AZURE_CLIENT_SECRET' .github/workflows/`,
	finding.CheckGHActionNPMTokenNotOIDC:       `grep -r 'NODE_AUTH_TOKEN\|NPM_TOKEN' .github/workflows/`,
	finding.CheckGHActionPyPITokenNotTrusted:   `grep -r 'PYPI_API_TOKEN\|TWINE_PASSWORD' .github/workflows/`,
	finding.CheckGHActionDockerPasswordSecret:  `grep -r 'DOCKER_PASSWORD\|DOCKERHUB_TOKEN' .github/workflows/`,
	...
	finding.CheckGitHubNoSAST:                  "grep -r 'codeql' .github/workflows/ || echo 'no codeql workflow'",
	finding.CheckGitHubNoVulnAlerts:            "gh api repos/{asset}/vulnerability-alerts",
	finding.CheckGitHubForkWorkflowApproval:    "gh api repos/{asset} --jq '.allow_forking'",
	finding.CheckGitHubDefaultTokenWrite:       "gh api repos/{asset} --jq '.permissions'",
	finding.CheckGitHubActionsUnrestricted:     "gh api repos/{asset}/actions/permissions",
	finding.CheckGitHubWebhookNoSecret:         "gh api repos/{asset}/hooks",
	finding.CheckGitHubOrgMFANotRequired:       "gh api orgs/{asset}  --jq '.two_factor_requirement_enabled'",
	finding.CheckGitHubSecretInCode:            "gh search code --repo {asset} 'AKIA OR sk_live OR ghp_'",
	finding.CheckGitHubPrivateKeyInRepo:        "gh search code --repo {asset} 'BEGIN PRIVATE KEY'",
