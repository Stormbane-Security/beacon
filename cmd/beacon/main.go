...
EXAMPLES:
  ...
  beacon scaffold    --id <id> [--param k=v]   Scaffold new security-hardened infrastructure
  beacon scaffold    --query "description"     AI-matched scaffold from natural language
  beacon catalog     [--provider <p>]          List available infrastructure patterns
  beacon backstage                             Generate Backstage software templates
...
func cmdScaffold(args []string) {
	...
	if query != "" && catalogID == "" {
		...
		if len(matches) == 0 {
			fatalf("no catalog entries match %q — run 'beacon catalog' to see all", query)
		}
		fmt.Fprintf(os.Stderr, "beacon: matched %d patterns for %q\n\n", len(matches), query)
		...
		fmt.Fprintf(os.Stderr, "\nbeacon: using top match: %s\n\n", catalogID)
	}

	if catalogID == "" {
		fatalf("usage: beacon scaffold --id <catalog-id> [--param key=value ...]\n       beacon scaffold --query \"I need a kubernetes cluster on GCP\"")
	}
	...
	if err != nil {
		if result != nil && len(result.MissingParams) > 0 {
			fmt.Fprintf(os.Stderr, "beacon: missing required parameters:\n")
			for _, name := range result.MissingParams {
				for _, p := range result.Entry.Params {
					if p.Name == name {
	...
	fmt.Fprintf(os.Stderr, "beacon: scaffolded %s — %d files\n\n", result.Entry.Name, len(result.Files))
...
func cmdBackstage() {
	...
	fmt.Fprintf(os.Stderr, "beacon: generated %d Backstage template files\n\n", len(files))
	for path, content := range files {
		fmt.Printf("--- %s ---\n%s\n\n", path, content)
	}
}
